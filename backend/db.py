...
class SQLiteStore:
    ...
    def _init_db(self) -> None:
        ...
        # Migration: rebuild table if it was created without UNIQUE on domain.
        ...
        with self._lock:
            with self._connect() as conn:
                row = conn.execute(
                    ...
                    "WHERE type='table' AND name='scrape_history'"
                ).fetchone()
                # sqlite3.Row supports index access; use column index 0 for 'sql'
                if row is None:
                    return
                ddl: str = row[0] or ""
                if "UNIQUE" in ddl.upper():
                    return  # already migrated — nothing to do
                # Rebuild keeping only the latest row per domain
                conn.executescript("""
                    BEGIN;
